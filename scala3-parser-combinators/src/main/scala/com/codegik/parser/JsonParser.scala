...
enum Json {
  ...
  override def toString: String = this match {
    ...
    case JNumber(v)  => if (v == v.toLong) v.toLong.toString else v.toString
    case JString(v)  => s"\"$v\""
...
object JsonParser {
  ...
  lazy val jValue: Parser[Json] = Parser(input =>
    token(choice(jNull, jBool, jNumber, jString, jArray, jObject)).run(input)
  )
